...
export class BookShopWithRxjsSubject extends AbstractSubject {

  // Observable - Subject
  private magazineSubject: Subject<string>
  = new Subject<string>();
  ...
  subscribe(observer: AbstractObserver): void {
    if(this.observerSubscriptionMap.has(observer) === false) {
      const subscription = this.magazineSubject
  ...
  notify(data: string): void {
    this.magazineSubject.next(data);
  }
}
